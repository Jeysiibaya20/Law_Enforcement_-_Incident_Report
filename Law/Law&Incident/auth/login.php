...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // Get user data
        $sql = "SELECT u.*, e.employee_id, e.first_name, e.last_name, e.email, e.status as employee_status
                FROM users u
                JOIN signup e ON user_id = user_id
        ...
        if ($user['employee_status'] !== 'Active') {
            throw new Exception('Your account is not active. Please contact HR.');
        }
        
        // 2FA handling: if enabled, send email code and require verification first
        $tfa = new TwoFactorAuth($pdo);
        if ($tfa->is2FAEnabled($user['user_id'])) {
            $code = $tfa->generateSMSCode();
            // Store as EMAIL type
            $tfa->storeEmailCode($user['user_id'], $code);
            // Send to employee email
            $tfa->sendEmailCode($user['email'], $code);
            // Set pending 2FA session and temp identity
            $_SESSION['pending_2fa_user'] = $user['user_id'];
            $_SESSION['pending_2fa_role'] = $user['role'];
            $_SESSION['pending_2fa_employee_id'] = $user['employee_id'];
            $_SESSION['pending_2fa_username'] = $user['username'];
            $_SESSION['pending_2fa_email'] = $user['email'];
            header('Location: verify_2fa.php');
            exit();
        } else {
            // Set session variables and finalize login
